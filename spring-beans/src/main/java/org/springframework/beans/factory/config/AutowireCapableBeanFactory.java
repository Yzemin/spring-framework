...
/**
 * {@link org.springframework.beans.factory}的扩展。
 * 接口由能够自动装配的bean工厂来实现，前提是他们想要为现有的bean实例公开这个功能。
 * <p>
 * BeanFactory的子接口不应该在普通的应用程序代码中使用:{@link org.springframework.beans.factory}
 * 或{@link org.springframework.beans.factory.ListableBeanFactory}用于典型用例。
 * 其他框架的集成代码可以利用这个接口来连接和填充Spring不控制生命周期的现有bean实例。
 * 例如，这对于WebWork Actions和 Tapestry Page对象特别有用。
 * 注意，这个接口不是由{@link org.springframework.context.ApplicationContext} facade，因为它很少被应用程序代码使用。
 * 也就是说，它也可以从应用程序上下文获得，可以通过ApplicationContext的方法访问。
 *
 * <p>你也可以实现{@link org.springframework.beans.factory}。接口，它公开了内部的BeanFactory，
 * 即使运行在一个 ApplicationContext中，以获得对AutowireCapableBeanFactory的访问:
 *
