...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	 ...
	 * 如果这个类的servlet在上下文中被命名为“test”，servlet使用的命名空间将解析为“test-servlet”。
	 */
	...
	/**
	 * WebApplicationContext的ServletContext属性的前缀。完成是servlet名。
	 */
	public static final String SERVLET_CONTEXT_PREFIX = FrameworkServlet.class.getName() + ".CONTEXT.";

	/**
	 * A任意数量的这些字符被认为是一个初始化参数字符串值中多个值之间的分隔符。
	 */
	private static final String INIT_PARAM_DELIMITERS = ",; \t\n";


	/**
	 * ServletContext属性来查找WebApplicationContext
	 */
	@Nullable
	...
	protected final void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		//重新保存LocaleContext和RequestAttributes
		initContextHolders(request, localeContext, requestAttributes);

		try {
			//DispatcherServlet处理请求入口
			...
			//重置LocaleContextHolder和RequestContextHolder 为默认状态
	...
	/**
	 * 子类必须实现这个方法来完成请求处理的工作，
	 * 接收GET、POST、PUT和DELETE的集中回调。
	 * <p>
	 * 这个契约本质上与通常覆盖的HttpServlet的 {@code doGet}或{@code doPost}方法相同。
	 * <p>
	 * 这个类拦截调用以确保异常处理和事件发布发生。
	 *
	 * @param request  current HTTP request
	 * @param response current HTTP response
	...
	/**
	 * CallableProcessingInterceptor实现，
	 * 初始化和重置 FrameworkServlet的上下文持有者，即LocaleContextHolder和RequestContextHolder。
	 */
